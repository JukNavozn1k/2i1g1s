...
begin
    ...
    repeat
        ...
        begin
            write('Введите первую точку (a >= -1.64): ');
            read(a);
            gotoxy(10,y+i);
            i:=i+1;
            end;
        ...
        m:=trunc(m2);
            ...
            RE:=abs(E/S1)*100;
            str(E:0:2,temp_str);
            absolute_error_str:='Абсолютная погрешность: ' + temp_str;
            str(RE:0:2,temp_str);
            relative_error_str:='Относительная погрешность: ' + temp_str + '%';
            str(S2:0:2,temp_str);
            area_str:='Результат: ' + temp_str; 
    flag:=true;
end;
